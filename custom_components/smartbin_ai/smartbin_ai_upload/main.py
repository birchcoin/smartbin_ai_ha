...
from .const import DEFAULT_API_URL, DEFAULT_MODEL, DEFAULT_TEXT_MODEL
...
class SmartBinUploadView(HomeAssistantView):
    ...
    url = "/api/smartbin_ai_upload/upload"
    name = "api:smartbin_ai_upload:upload"
    ...
    async def post(self, request: web.Request) -> web.Response:
        ...
        target_dir = Path("/config/www/bins") / folder_id
        ...
        if mode == "remove":
            script_name = "smartbin_ai_analyze_and_remove"
        else:
            script_name = "smartbin_ai_append_and_analyze"
...
class SmartBinAnalysisLogView(HomeAssistantView):
    ...
    url = "/api/smartbin_ai_upload/analysis_log"
    name = "api:smartbin_ai_upload:analysis_log"
    requires_auth = True

    async def get(self, request: web.Request) -> web.Response:
        hass: HomeAssistant = request.app["hass"]
        log_path = Path("/config/ANALYSIS_DEBUG.log")
...
class SmartBinLaunchView(HomeAssistantView):
    ...
    url = "/smartbin_ai_upload/launch"
    name = "smartbin_ai_upload:launch"
    requires_auth = False

    async def get(self, request: web.Request) -> web.Response:
        hass: HomeAssistant = request.app["hass"]
        bin_id = request.query.get("bin", "smartbin_ai_001")
...
class SmartBinLaunchRemoveView(HomeAssistantView):
    ...
    url = "/smartbin_ai_upload/launch_remove"
    name = "smartbin_ai_upload:launch_remove"
    requires_auth = False
...
class SmartBinLaunchSimpleView(HomeAssistantView):
    ...
    url = "/smartbin_ai_upload/launch_simple"
    name = "smartbin_ai_upload:launch_simple"
    requires_auth = False
...
<body>
  ...
  <script>
    ...
    function uploadImage(file) {{
      ...
      fetch('/api/smartbin_ai_upload/upload', {{
        method: 'POST',
        body: formData
      }})
      .then(async response => {{
...
async def analyze_bin_image_service(call: ServiceCall) -> None:
    ...
    try:
        ...
        debug_image_path = f"/config/www/bins/DEBUG_ai_input_{os.path.basename(image_path)}"
...
async def async_setup(hass: HomeAssistant, config: dict) -> bool:
    ...
    async def remove_image_service(call: ServiceCall) -> None:
        ...
        if filename in images:
            ...
            folder_id = bin_id.replace("smartbin_", "")
            file_path = Path(f"/config/www/bins/{folder_id}/{filename}")
            if file_path.exists():
    ...
    async def sync_images_from_disk(_event) -> None:
        ...
        for bin_id in DEFAULT_BINS:
            entry = _get_bin_entry(hass, bin_id)
            folder_id = bin_id.replace("smartbin_", "")
            folder = Path("/config/www/bins") / folder_id
            if not folder.exists():
                continue
